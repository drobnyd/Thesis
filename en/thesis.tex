...
\TODO{To demonstrate the functionality that is independent of other Manta Flow parts, we attach scripts doing metadata extraction from the modeling tools.
Due to the licensing reasons it has only partial functionality consisting of extracting the physical and business objects with their metadata and visualizing them as a graph in an output file. 
No data lineage though is going to be presented in the demo version.}
...
System consists of two items - one is containing the models from which metadata will get extracted and a .ini file where connection specifications to physical models are defined.  \TODO{describe .ini file or not as it won't have any effect on the demo.} \\

For each system in input a property file specifying a name of the system folder to be processed must be present. \\ 

To proceed the extraction user runs one of the executable files provided \TODO{TODO}.sh for Unix-like, \TODO{TODO}.bat for Windows machines respectively.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Permissions \\
		\TODO{By owner assigned to an entity permissions are realized.} \\
		\item Keys \\
