...
\TODO{justification of each of property?}
...
First thing to do, we will try the ideal scenario and check whether the target is in the input directory. If yes, we are done with this one. \TODO{example} 
...
And why would we want to request another metadata when that is just what we are extracting from physical data models?
...
A .ini files is used for that where sections are named by physical models that they correspond to and connection details are specified inside a section. 
...
The nice thing about them is that they are not that flexible and once we know whether we deal with native or JDBC respectively we know what exact structure expect. 
...
But there is a problem that is common for both of the approaches, namely that there is no link between the connection file and a model that is result of reverse-engineering of the connection. So we will need to create a workaround. \TODO{how it is solved, also .ini file can be the solution}
...
Further division of each of the parts is that they are split the system into four modules. 
...
\begin{itemize}
	...
	\item Manta Flow \\
	The external part creating a graph of data lineage. 
\end{itemize}
\TODO{A figure showing cooperation of the most components }